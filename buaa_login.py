...
class BuaaLogin(object):
    host_login_page_url = 'https://gw.buaa.edu.cn/srun_portal_pc?ac_id=1&theme=buaa'  # 获取 ip
    get_challenge_url = 'https://gw.buaa.edu.cn/cgi-bin/get_challenge'  # 获取 token
    log_in_url = 'https://gw.buaa.edu.cn/cgi-bin/srun_portal'  # 登录 & 注销
    get_login_info_url = 'https://gw.buaa.edu.cn/cgi-bin/rad_user_info'  # 获取登录后的信息
    new_headers = {
        ...
        'Cookie': 'lang=zh-CN',
        'Host': '10.200.21.4',
        'Referer': 'https://gw.buaa.edu.cn/srun_portal_pc?ac_id=1&theme=buaa',
    ...
    def get_ip(self):  # 获取 ip 地址
        ...
        if ip_address:
            print(f"IP address getted - <{ip_address}>")
            return ip_address
        else:
            print('Getting IP address failed')
            return None

    def get_challenge(self):  # 获取 taken
        get_challenge_params = {
            "callback": 'jQuery112406034969373332884_' + str(int(time.time() * 1000)),  # 这里的字符串少了一位
        ...
        if challenge:
            print('Successfully getting challenge')
            return challenge
        else:
            print('Getting challenge failed')
            return None
...
#        print(log_in_response.text)
    ...
    def get_login_info(self):  # 获取表示登录状态的代码
        params = {
            ...
            'callback': 'jQuery1124015674951555275918',
            '_': str(int(time.time()))
        }
        ...
        if error_info == 'ok':
            ...
            user_name = re.search(r'"user_name":"(\w+)"', str_login_info).group(1)
            ...
            print('Getting login info failed - ' + f'<{error_info}>')

    def log_out(self):  # 注销
        params = {
            'callback': 'jQuery1124006940033722666095',
            'action': 'logout',
            'ac_id': '1',
            'ip': self.ip,
            'username': self.username,
